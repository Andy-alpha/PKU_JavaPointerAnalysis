...
public class Solver {
    private pascal.taie.World world;
    // Why use set instead of TreeSet?
    private Set<Integer> all_allocIds = new TreeSet<>();

    /**
     * We use MySignature(String) here to index the points-to-sets, where each
     * variable owns
    ...
    /**
     * Each "key"(String, or signature) represents a variable in the dataflow graph,
     * and
     * the "value" associated with it is a set that contains the other variables
     * which the
     * object-pointer-flow will propagate to, i.e. if we take the key as a Rvalue in
     * some
    ...
    public final int clone_depth = 256;
     ...
     * If two fields have different signatures, they are definitely not the same
     * field.
     * If two fields have the same field signature, they have the same field name,
     * type and base class type.
     * So, for all JField with the same signature, we only have to store one
     * instance.
     * Why don't I use a set of Strings instead? JField contains more information,
     * such as whether it is an Instance field or a Static field, which should be
     * treated differently.
     */
    ...
    private void AddEdge(String from, String to) {
        if (from.equals(to))
            return; // In case the two symbols are the same thing.
    ...
    public void Solve(World world_) {
        ...
        try {
            // 等待Run()执行，最多等待55秒
            future.get(30, TimeUnit.SECONDS); // 如果超时，将抛出TimeoutException
            Answer(false); // 如果成功，执行Answer(false)
        } catch (Exception e) {
            Answer(true);
            return;
        } finally {
    ...
    public void Initialize() {
        ...
        for (JClass jClass : world.getClassHierarchy().allClasses().toList()) {
            ...
            // if(jClass.toString().contains("<java") || jClass.toString().contains("<com"))
            // continue;
            for (JField jField : jClass.getDeclaredFields()) {
                ...
                for (CharSequence str : new CharSequence[] { "<java", "<com", "<sun", "<jdk" }) {
                    if (jField.toString().contains(str)) {
                        isLib = true;
                        break;
                    }
                }
    ...
    public void InitConstraints(JMethod method) {
        ...
        for (Stmt statement : statements) {
            if (statement instanceof Invoke invoke_stmt) {
                ...
                if (isLibrary(newMethod))
                    continue; // Important!!!!!!!!!!!!!!!!!!!!!!!!!
                String signature = invoke_stmt.getMethodRef().toString();

                // These statements may throw exceptions if the argument is not a constant,
                // handle them?
                ...
                AddNewConstraints(GenMySignature(new_stmt.getLValue(), cur_clone_depth), allocId); // map temp$k(heap
                                                                                                   // var) to allocId
                ...
                /**
                 * @// TODO: Implement unrolling many times
                 * Can you use Tai-e CFG module to implement it
                 *
    ...
    private void DealWithInvokeStatement(Invoke invoke_stmt, int cur_clone_depth) {
        ...
        for (JClass subClass : world.getClassHierarchy().getAllSubclassesOf(declaringClass)) {
            if (invokedMethod.getSignature().contains("void <init>"))
                break; // Brutally skip init functions.
            if (subClass.toString() == declaringClass.toString())
                continue;
            ...
            if (subMethod == null)
                continue; // The subClass did not override this method.
    ...
    private void DealWithNonAbstractInvokeStatement(
        ...
        // Can put in debug statements
        if (!method.isStatic()) {
            /**
             * If this function belongs to a Var, then %this should be connected with this
             * Var.
             * (Important!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!)
             */
     ...
     *         @// TODO: 2024/11/27 For now this function is only a counter
     */
     ...
     * Even if this var does not have this field, the fake signature will be
     * generated.
     * 
